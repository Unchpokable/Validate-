...
#   Usage: ./scripts/build_and_test.sh [Debug|Release]
# ============================================================
...
# ── Header ────────────────────────────────────────────────────
banner "Validate! - Build & Test Runner   [Config: $BUILD_CONFIG]"
echo ""
...
for candidate in "$BUILD_DIR/tests/$BUILD_CONFIG" "$BUILD_DIR/tests"; do
    if [ -f "$candidate/test_assert" ] || [ -f "$candidate/test_assert.exe" ]; then
        BIN_DIR="$candidate"
        break
    fi
done
...
for SUITE in "${SUITES[@]}"; do
    ...
    # Find binary (with or without .exe extension)
    EXE=""
    for EXT in "" ".exe"; do
        candidate="$BIN_DIR/$SUITE$EXT"
        if [ -f "$candidate" ]; then
            EXE="$candidate"
            break
        fi
    ...
    if [ -z "$EXE" ]; then
        echo -e "  ${RED}[ERROR] Binary not found in: $BIN_DIR${NC}"
        SUITE_NAMES+=("$SUITE")
        SUITE_PASSED+=(0)
        SUITE_FAILED+=(0)
    ...
    # Run the test binary; stream output to terminal AND capture to temp file
    SUITE_OUT="$(mktemp)"
    ...
    SUITE_F=$(grep '  FAILED  ] ' "$SUITE_OUT" 2>/dev/null \
        | grep ' listed below' \
        | sed 's/.*FAILED  ] \([0-9][0-9]*\).*/\1/' \
        | head -1)
    SUITE_F="${SUITE_F:-0}"

    # Collect individual failed test names:
    #   summary-section lines have no " ms)" timing suffix
