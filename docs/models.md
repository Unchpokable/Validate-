...
**Compile-time альтернатива.** Если полный набор правил известен на этапе компиляции и важно избежать аллокаций под `std::function`, есть `vd::static_model<T, Rules...>` — модель, у которой набор правил является частью типа (`std::tuple<Rules...>`), а не значения. См. [static-model.md](static-model.md).
